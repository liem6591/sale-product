...
class AmazonOrder {
	...
	function asynFbaOrderItems($accountId){
		...
		//获取未获取明细的FBA订单信息
		$orders = $System->exeSqlWithFormat("
		SELECT * FROM sc_order so WHERE so.FULFILLMENT_CHANNEL = 'AFN' AND so.ORDER_ID NOT IN (
				SELECT soi.Order_ID FROM sc_order_item soi
		)  and so.account_id = '$accountId' LIMIT 0 , 60",array()) ;
		
		foreach ($orders as $order) {
			$orderId = $order['ORDER_ID'] ;
			$this->getOrderItems($orderId, $accountId) ;
		}
	}
